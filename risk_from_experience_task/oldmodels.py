...
class Constants(BaseConstants):
    ...
    values_uncertain = [(i, j, z) for i, j, z in zip(
        [10, 6, 7, 8], [0, 0, 1, 4], [0.5, 0.5, 0.5, 0.25])]
    values_certain = [(6, 4), 3, 4, 5]
                    ...
                    rep_for_pair.append(
                        {'pair': d + 1,
                         'values': {
                             ...
                             'B': pairs_values_and_probs[d]['B'] if d > 0 else rnd.choice(list(pairs_values_and_probs[d]['B']))
                         },
                         'order': o}
                    )
...
class Player(BasePlayer):
    ...
    def set_payoff(self):
        self.payoff = sum(self.participant.vars['pair_payoffs'])
